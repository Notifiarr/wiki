...
## Q. How to allow Notifiarr through Cloudflare (WAF RULE)
...
## Q. How to use a Cloudflare tunnel with Notifiarr

> This assumes that you already have a cloufdlare tunnel set up on your system. If you want to get started with cloudflare tunnels follow this guide first: https://www.youtube.com/watch?v=RUJy9fjoiy4
{.is-info}
...
If you want to increase the security of your local server follow these steps to only allow the Notifiarr service access to your "notifiarr.yourdomain.com" subomain. There are to steps to this.
...
5. Turn on "Skip identy provider selection if only one is configured" at the bottom of the page and click **next**.
...
7. Click **next**
1. On the next page don't make any changes and click **Add aplication**
...
This works very well for your services such as Radarr and Sonarr, for example. Obviously the Notifiarr service won't be able to verfy itself via email so we need to make an exception for Notifiarr next.
...
- `Application Name`: Whatever you want. I usually call this "Domain Notifiar Bypass"
...
- `Policy Name`: Whatever you want. I usually call this "Allow IP"
...
That's it! You now gave one spefic IP address (Notifiarr service) permission to BYPASS the cloudflare email authorization check. Only the Notifiarr service will have access to "notifiarr.yourdomain.com". (You will still have access to it as well via email authorization).
